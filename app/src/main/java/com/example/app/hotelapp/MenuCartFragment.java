package com.example.app.hotelapp;
...
public class MenuCartFragment extends Fragment{
    ...
    public void initOrderedList(){
        ...
        if(null != tmpArray) {
            for (int i = 0; i < 10; i++) {
                if(0 != tmpArray[i]){
                    Item item = new Item(i + 1, "Item #" + Integer.toString(i + 1), 10.0 * (i + 1), tmpArray[i], MainActivity.imgArr[i]);
    ...
    private void resetOrderListCart(){
        ...
        for (int i = 0; i < 10; i++) {
            Item item = new Item(i + 1, "Item #" + Integer.toString(i + 1), 10.0 * (i + 1), 0, MainActivity.imgArr[i]);
